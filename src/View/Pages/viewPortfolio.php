<?php $this->title = "NG Photographie - Portfolio !"; ?>
<!-- Titre de la page -->

<section class="banner_page">
    <div class="banner_page_img">
        <img src="Contents/Illustration/banner-portfolio.jpg" alt="image de mise en page portfolio">
    </div>
</section>

<section id="portfolio">
    <h1>Portfolio</h1>

    <div class="portfolio_presentation">
        <div class="portfolio_presentation_img">
            <img src="Contents/Illustration/portrait-index.png" alt="image de présentation portrait">
        </div>
        <div class="portfolio_presentation_text">
            <h2>Portrait</h2>
            <p>Vous pouvez trouver ici mes dernières photos de portraits en date.</p>
            <a class="button" href="Portfolio-Portrait">Voir les photos</a>
        </div>
    </div>

    <div class="portfolio_presentation">
        <div class="portfolio_presentation_img">
            <img src="Contents/Illustration/animals-index.png" alt="image de présentation animalier">
        </div>

        <div class="portfolio_presentation_text">
            <h2>Animalier</h2>
            <p>Vous pouvez trouver ici mes dernières photos d'animaux en date.</p>
            <a class="button" href="Portfolio-Animal">Voir les photos</a>
        </div>
    </div>

    <div class="portfolio_presentation">
        <div class="portfolio_presentation_img">
            <img src="Contents/Illustration/landscape-index.png" alt="paysage">
        </div>

        <div class="portfolio_presentation_text">
            <h2>Paysage</h2>
            <p>Vous pouvez trouver ici mes dernières photos de paysages en date.</p>
            <a class="button" href="Portfolio-Landscape">Voir les photos</a>
        </div>
    </div>
</section>